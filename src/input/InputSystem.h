...
public:
	...
	/* Bind a input (key/button) to a float event */
	void bindInput(int input, Event<weak_ptr<Entity>, float>* event, weak_ptr<Entity> e);
	...
	/* Create an axis from a single axis input and bind to event */
	void createAxis(int input, Event<weak_ptr<Entity>, float>* event, weak_ptr<Entity> e);
